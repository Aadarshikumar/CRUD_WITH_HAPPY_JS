...
const init = async () => {
    ...
    server.route({
        ...
        handler: async(req, h) => {
            // console.log(data)
            try {
                let data = await knex('happy_js').where({email:req.payload.email})
    ...
    server.route({
        ...
        handler: async (req, res) => {
            ...
            try {
                console.log(req.payload);
                const data = await knex('happy_js').where({email: req.payload.email}).update(req.payload)
    ...
    server.route({
        ...
        handler: async (req, h) => {
            ...
            try {
                const data = await knex('happy_js').where({email: req.payload.email}).del(req.payload)
                return 'data deleted'
            } catch (err) {
                return err;
